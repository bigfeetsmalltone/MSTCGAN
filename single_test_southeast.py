...
#         img[:, self.patch_size*self.patch_size, :, :] = img[:, self.patch_size*self.patch_size, :, :] / self.pred_seq_len
    ...
    def read_image_list(self, image_dir, mode, seed):
        ...
        sequences = json.load(open(os.path.join(image_dir, 'sequences_24_south_east_202006.json'), 'r'))
        return sequences
...
def constant_padding_image(image):
    ...
    # print(temp.shape)
    # print(temp.shape)
    m = torch.nn.ReflectionPad2d((21, 21, 18, 18))
...
data_path = './github_south_east_area_samples'
train_seq_len = 8
pred_seq_len = 16
time = '202006'
...
#                               pin_memory=False)
    with torch.no_grad():
        ...
        #         seq_images = seq_images.detach().cpu().numpy()*255
            ...
            # print(seq_images.shape)
            # print(seq_labels.shape)
            write_picture('./results' + model_index + '_south_east_' + time + '/' + str(i_batch) +  '/groud_truth/', seq_labels)
            
            # pre_label = seq_images[0][-1][0].clone()
            # for i in range(len(pred_images)):
            #     pred_images[i] = hist_match_reverse(pred_images[i], pre_label.detach().cpu().numpy())

            write_picture('./results' + model_index + '_south_east_' +  time + '/' + str(i_batch) + '/pred/', pred_images)
            write_picture_in('./results' + model_index + '_south_east_' + time + '/' + str(i_batch) + '/in/', seq_images[0,:,0,:,:].numpy()*255)
